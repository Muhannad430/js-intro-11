...
with their quantities as key-value pairs and returns the total price of the given items based on the price list below.
1 Apple is $2.00​
1 Orange is 3.29
1 Mango is $4.99
1 Pineapple $5.25
...
function calculateTotalPrice1(obj) {
    ...
    for (let i in obj) {
        if (i === 'apple') {
            total += obj[i] * prices.apple;
        }
        else if (i === 'orange') {
            total += obj[i] * prices.orange;
        }
        else if (i === 'mango') {
            total += obj[i] * prices.mango;
        }
        else if (i === 'pineapple') {
            total += obj[i] * prices.pineapple;
        }
        else {
            if (obj[i] === 0) {
                return '';
...
/*
Requirement:
Write a function named calculateTotalPrice2() which takes an object of some shopping items 
with their quantities as key-value pairs and returns the total price of the given items based on the price list below.
1 Apple is $2.00​
1 Orange is 3.29
1 Mango is $4.99
...
function calculateTotalPrice2(obj) {
    ...
    for (let i in obj) {
        if (i === 'Apple') {
            total += (obj[i] * prices.apple) - (Math.floor(obj[i] / 2));
        }
        else if (i === 'Orange') {
            total += obj[i] * prices.orange;
        }
            ...
            if (obj[i] >= 4) total += (obj[i] * prices.mango) - (freeMangos * prices.mango)
            else total += obj[i] * prices.mango
        }
        else if (i === 'Pineapple') {
            total += obj[i] * prices.pineapple;
        }
...
function doubleOrTriple(array, boolean) {
    return array.map((ele) => boolean ? ele * 2 : ele * 3);
}
